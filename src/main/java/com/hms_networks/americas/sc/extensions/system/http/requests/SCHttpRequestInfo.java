...
public abstract class SCHttpRequestInfo {
  ...
  /**
   * Performs an HTTP(s) request (using the configured method) to the specified URL using the
   * specified request header and body.
   *
  ...
  public String doRequest(String url, String header, String body)
    ...
    return SCHttpUtility.httpRequest(url, header, body, getMethod(), null);
  }

  /**
   * Performs an HTTP(s) request (using the configured method) to the specified URL using the
   * specified request header and body, then outputs the response to the specified file. If the
   ...
   * @since 1.12.0
   */
  public String doRequest(String url, String header, String body, String outputFile)
      throws EWException,
          IOException,
          SCHttpEwonException,
          SCHttpAuthException,
          SCHttpConnectionException,
          SCHttpUnknownException {
    return SCHttpUtility.httpRequest(url, header, body, getMethod(), outputFile);
  }
}
